...
<template>
  <button @click="add">聚点</button>
  <button @click="remove">取消第一点</button>
  <button @click="getPointPostion">定位到第五个</button>
  <button @click="update">跟新第五个点的位置和图标</button>
  <button @click="starts">生成小车</button>
  <wd-switch v-model="checked" @change="show" />
  <div relative>
...
const a = getCurrentInstance()!.vnode
console.log(a)
const mapRef = ref<HTMLElement>(null)
const checked = ref(false)
const { initMap, map, markerClusterGroup, start, addLayers, removeLayers } = useMap()
const defaultList = Array.from({ length: 100 }, (_, i) => {
  return {
    title: '这是一个实例' + i,
    icon: 'https://vitejs.cn/vite3-cn/logo.svg',
    lat: 31.5 + i * Math.random() * 0.001,
    lng: 120.3 + i * Math.random() * 0.001,
  }
})
const list = markerClusterGroup(defaultList) /* 生成grop */

function show({ value }) {
  const nodeTitleList = document.querySelectorAll('.title')
  nodeTitleList.forEach((el: HTMLElement) => (el.style.display = value ? 'block' : 'none'))
}
onMounted(() => {
  if (mapRef.value) {
    initMap(mapRef.value as HTMLElement)
  }
})
/* 添加点 */
function add() {
...
function update() {
  list[5].getIcon().options.html = `<div style="color:red">这是第五个点</div>`
  list[5].setLatLng([31.6, 120.6])
  list[5].setIcon(list[5].getIcon())
}

function getPointPostion() {
  map.value.setView(list[5]._latlng, 18)
}
